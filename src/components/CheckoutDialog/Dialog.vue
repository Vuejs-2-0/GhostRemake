<template>
<div class="w-full">
  <Dialog @update:open="dialogUpdated"  >
        ...
        <template v-if="page=='summary'">
          ...
          <div class="w-full">
            <div v-for="entry in productEntries" :key="entry.entry_id" class="w-full grid grid-cols-4 gap-2 mt-4 justify-center items-center text-center">
              ...
              <p class="text-left pl-1 col-span-2">{{ entry?.metadata?.label }} {{ entry?.metadata?.bracelets.effect }} {{ entry?.metadata?.bracelets.size }} {{ entry?.metadata?.bracelets.comment }}</p>
          ...
          <hr class="my-6 w-full">
            ...
            <p>中文姓名：<span class="text-sm font-light">{{ dry_run_result.form.chineseName }}</span></p>
            <p>英文姓名：<span class="text-sm font-light">{{ dry_run_result.form.chineseName }}</span></p>
            <p>联络号码：<span class="text-sm font-light">{{ dry_run_result.form.phoneNumber }}</span></p>
            <p>电邮地址：<span class="text-sm font-light">{{ dry_run_result.form.email }}</span></p>
